...
describe("DefaultFileUploadChild", function () {
    ...
    it("shows file info if upload completed", function () {
        var wrapper = mount(React.createElement(DefaultFileUploadChild, { documentType: documentTypes.required, uploadState: FileUploadState.Complete, document: { name: "bar", fileSize: 100 }, baseUrl: "", token: "" }));
        expect(wrapper
            .find(".textTitle")
            .at(0)
            .text()).toEqual("somename");
    ...
    it("shows error text if upload failed", function () {
        var wrapper = mount(React.createElement(DefaultFileUploadChild, { documentType: documentTypes.optional, uploadState: FileUploadState.Error, error: "something wrong", baseUrl: "", token: "" }));
    ...
    it("shows download link if template file is present", function () {
        var wrapper = mount(React.createElement(DefaultFileUploadChild, { documentType: documentTypes.required, baseUrl: "", token: "" }));
        expect(wrapper
            .find(".downloadLink")
            .at(0)
